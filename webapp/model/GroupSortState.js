sap.ui.define([
	...
	return BaseObject.extend("com.lesaffre.advance.model.GroupSortState", {
		 ...
		 * @alias com.lesaffre.advance.model.GroupSortState
		 */
		...
		/**
		 * Groups by Advam, or resets the grouping for the key "None"
		 *
		 * @param {string} sKey - the key of the field used for grouping
		 * @returns {sap.ui.model.Sorter[]} an array of sorters
		 */
		group: function (sKey) {
			...
			if (sKey === "Advam") {
				...
				this._oViewModel.setProperty("/sortBy", "Advam");

				aSorters.push(
					new Sorter("Advam", false,
						this._fnGroupFunction.bind(this))
				);
			} else if (sKey === "None") {
				// select the default sorting again
				this._oViewModel.setProperty("/sortBy", "Advtx");
			}
